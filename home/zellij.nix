      ...
      default_tab_template {
        pane size=1 borderless=true {
          plugin location="${zjstatusWasm}" {
            ...
            format_left   "#[fg=$sapphire] #[bg=$sapphire,fg=$crust,bold] #[bg=$surface1,fg=$text,bold] {session}  {mode} {tabs}"
            format_center "{notifications}"
            format_right  "#[fg=$flamingo]#[fg=$crust,bg=$flamingo] #[bg=$surface1,fg=$flamingo,bold] {command_user}@{command_host}#[fg=$surface1] #[fg=$maroon]#[bg=$maroon,fg=$crust]󰃭 #[bg=$surface1,fg=$maroon,bold] {datetime}#[fg=$surface1]"
            format_space  ""
            ...
            tab_normal              "#[fg=$surface2] #[bg=$surface2,fg=$text,bold] {name}{floating_indicator}#[bg=$surface2,fg=$surface1] █#[bg=$surface1,fg=$text,bold] {index} #[fg=$surface1]"
            tab_normal_fullscreen   "#[fg=$surface2] #[bg=$surface2,fg=$text,bold] {name}{fullscreen_indicator}#[bg=$surface2,fg=$surface1] █#[bg=$surface1,fg=$text,bold] {index} #[fg=$surface1]"
            tab_normal_sync         "#[fg=$surface2] #[bg=$surface2,fg=$text,bold] {name}{sync_indicator}#[bg=$surface2,fg=$surface1] █#[bg=$surface1,fg=$text,bold] {index} #[fg=$surface1]"
            tab_active              "#[fg=$surface1] #[bg=$surface1,fg=$text,bold] {name}{floating_indicator}#[bg=$surface1,fg=$peach] █#[bg=$peach,fg=$crust,bold] {index} #[fg=$peach]"
            tab_active_fullscreen   "#[fg=$surface1] #[bg=$surface1,fg=$text,bold] {name}{fullscreen_indicator}#[bg=$surface1,fg=$peach] █#[bg=$peach,fg=$crust,bold] {index} #[fg=$peach]"
            tab_active_sync         "#[fg=$surface1] #[bg=$surface1,fg=$text,bold] {name}{sync_indicator}#[bg=$surface1,fg=$peach] █#[bg=$peach,fg=$crust,bold] {index} #[fg=$peach]"
            tab_separator           " "
